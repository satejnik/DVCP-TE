function cancel = progressbar( label, varargin )
%progressbar: add, remove or update an entry on the multi waitbar
%
%   progressbar(LABEL,VALUE) adds a waitbar for the specified label, or
...
%
%   progressbar(LABEL,COMMAND,VALUE,...) passes one or more command/value
...
%
%   cancel = progressbar(LABEL,VALUE) also returns whether the user has
...
%
%   progressbar('CLOSEALL') closes the waitbar window.
%
%   Example:
%   progressbar( 'CloseAll' );
%   progressbar( 'Task 1', 0 );
%   progressbar( 'Task 2', 0.5, 'Color', 'b' );
%   progressbar( 'Task 3', 'Busy');
%   progressbar( 'Task 1', 'Value', 0.1 );
%   progressbar( 'Task 2', 'Increment', 0.2 );
%   progressbar( 'Task 3', 'Reset' ); % Disables "busy" mode
%   progressbar( 'Task 3', 'Value', 0.3 );
%   progressbar( 'Task 2', 'Close' );
%   progressbar( 'Task 3', 'Close' );
%   progressbar( 'Task 1', 'Close' );
%
%   Example:
%   progressbar( 'Task 1', 0, 'CancelFcn', @(a,b) disp( ['Cancel ',a] ) );
%   for ii=1:100
%      abort = progressbar( 'Task 1', ii/100 );
%      if abort
...
%   Example:
%   progressbar( 'CloseAll' );
%   progressbar( 'Red...',    7/7, 'Color', [0.8 0.0 0.1] );
%   progressbar( 'Orange...', 6/7, 'Color', [1.0 0.4 0.0] );
%   progressbar( 'Yellow...', 5/7, 'Color', [0.9 0.8 0.2] );
%   progressbar( 'Green...',  4/7, 'Color', [0.2 0.9 0.3] );
%   progressbar( 'Blue...',   3/7, 'Color', [0.1 0.5 0.8] );
%   progressbar( 'Indigo...', 2/7, 'Color', [0.4 0.1 0.5] );
%   progressbar( 'Violet...', 1/7, 'Color', [0.8 0.4 0.9] );
%
...
%   Copyright 2007-2013 The MathWorks Ltd
    ...
    if ~ischar( label )
        error( 'progressbar:BadArg', 'LABEL must be the name of the progress entry (i.e. a string)' );
    end

    % Try to get hold of the figure
    if isempty( figh ) || ~ishandle( figh )
        figh = findall( 0, 'Type', 'figure', 'Tag', 'progressbar:Figure' );
        if isempty(figh)
            figh = iCreateFig();
        else
            figh = handle( figh(1) );
        ...
        for ii=1:numel( params )
            switch upper( params{ii} )
                ...
                case {'RELABEL', 'UPDATELABEL'}
                    ...
                    if ~ischar( values{ii} )
                        error( 'progressbar:BadString', 'Value for ''Relabel'' must be a string.' );
                    end
                    if ismember( values{ii}, {entries.Label} )
                        error( 'progressbar:NameAlreadyExists', 'Cannot relabel an entry to a label that already exists.' );
                    end
                ...
                case {'CANCANCEL'}
                    if ~ischar( values{ii} ) || ~any( strcmpi( values{ii}, {'on','off'} ) )
                        error( 'progressbar:BadString', 'Parameter ''CanCancel'' must be a ''on'' or ''off''.' );
                    end
                ...
                case {'CANCELFCN'}
                    if ~isa( values{ii}, 'function_handle' )
                        error( 'progressbar:BadFunction', 'Parameter ''CancelFcn'' must be a valid function handle.' );
                    end
                ...
                case {'CLOSE','DONE'}
                    ...
                    break;
                otherwise
                    error( 'progressbar:BadArg', 'Unrecognised command: ''%s''', params{ii} );
...
while idx <= nargin
    param = varargin{idx};
    if ~ischar( param )
        error( 'progressbar:BadSyntax', 'Additional properties must be supplied as property-value pairs' );
    end
...
if isempty( params )
    error( 'progressbar:BadSyntax', 'Must specify a value or a command' );
end
end % iParseInputs
...
% existing ones
delete( timerfind('Tag', 'progressbarTimer') );
myTimer = timer( ...
    'TimerFcn', @(src,evt) iTimerFcn(f), ...
    'Period', 0.02, ...
    'ExecutionMode', 'FixedRate', ...
    'Tag', 'progressbarTimer' );
...
lightColor = [1 1 1];
badColorErrorID = 'progressbar:BadColor';
...
if ischar(baseColor)
    ...
    elseif isa( baseColor, 'double' )
        if any(baseColor>1) || any(baseColor<0)
            error( 'progressbar:BadColorValue', 'Color values must be in the range 0 to 1 inclusive.' );
        end
    else
        error( badColorErrorID, badColorErrorMsg );
    end
